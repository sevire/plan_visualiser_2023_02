...
file_format_test_cases = [
    {
        ...
        'test_files': [
            {
                'file_data': file_data_01,
                'expected_result_file': expected_result_file_01,
                'expected_result_activity': None  # As we expect an error reading the file, there will be no activities
            },
            {
                'file_data': file_data_02,
...
class TestTechnicalFileFormat(TestCase):
    ...
    def test_technical_file_read(
        ...
        if file_reader == ExcelXLSFileReader:
            file_reader = file_reader(file_data['sheet_name'])
            file_name = os.path.join(EXCEL_PLAN_FILE_FOLDER, file_data['file_name'])
